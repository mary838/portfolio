;( function ( $ , window , document , undefined ) {
	...
	$.extend( Lightboxed.prototype , {
		...
		registerEvents : function ( scope ) {
			...
			if ( scope == 'lightbox' ) {
				this.lightbox.close.off('click').on( 'click' , function ( event ) { that.close.call( that , event ); } );
				this.lightbox.next.off('click').on( 'click' , function ( event ) { that.next.call( that , event ); } );
				this.lightbox.prev.off('click').on( 'click' , function ( event ) { that.prev.call( that , event ); } );
